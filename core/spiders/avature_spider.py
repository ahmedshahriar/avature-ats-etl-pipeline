...
class AvatureSpider(scrapy.Spider):
    ...
    def parse_listing(self, response):
        ...
        for a in soup.select('article.article--result a[href*="/JobDetail/"]'):
            ...
            job_url = urllib.parse.urljoin(current_url, href)
        ...
        if next_a and next_a.get('href'):
            next_link = urllib.parse.urljoin(current_url, next_a['href'])
        else:
            for a in soup.find_all('a', href=True):
                href = a['href']
                if 'jobOffset=' in href:
                    next_link = urllib.parse.urljoin(current_url, href)
                    break
        if next_link:
    ...
    def parse_job(self, response):
        ...
        if meta and meta.get('content'):
            item['job_id'] = meta['content'].strip()
        else:
        ...
        if job_id:
            identity = f"{host}|{portal}|{job_id}|{item['source_url']}".encode("utf-8")
        else:
            identity = f"{host}|{portal}|{item['source_url']}".encode("utf-8")
        ...
        if not title:
            banner = soup.select_one('h2.banner__text__title') or soup.select_one(
                'h2.banner__text__title--0') or soup.select_one('h2.banner__text__title--3')
            if banner:
